"""Input/output parsing helpers for Call Me Maybe.

The parser layer is intentionally boring and strict:
- read JSON safely
...
def read_json_file(path: str | Path) -> Any:
    ...
    try:
        ...
        raise ParserError(f"permission denied while reading: {file_path}") from exc
...
def validate_list(data: Any, model: type[T], source_name: str) -> list[T]:
    ...
    for index, item in enumerate(data):
        try:
            ...
            raise ParserError(f"invalid item at index {index} in {source_name}: {exc}") from exc
    return validated
...
def write_results(path: str | Path, results: list[FunctionCallResult]) -> None:
    ...
    try:
        ...
        raise ParserError(f"cannot write output file {output_path}: {exc}") from exc
