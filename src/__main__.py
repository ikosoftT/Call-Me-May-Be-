...
def main() -> int:
    ...
    for item in project_input.prompts:
        ...
        func_name = choose_function_name(model, item.prompt, project_input.functions)

        results.append(FunctionCallResult(prompt=item.prompt,name=func_name, parameters={}))
    try:
